/*
* serdes_gpio.c 
*   gpiolib support forn different serdes chip	
*
...
static int serdes_gpio_direction_out(struct gpio_chip *chip, unsigned int offset, int value)
{
    struct serdes_gpio *serdes_gpio = gpiochip_get_data(chip);
    struct serdes *serdes = serdes_gpio->parent->parent;
    int ret;
...
static int serdes_gpio_probe(struct platform_device *pdev)
{
    ...
    serdes_gpio = devm_kzalloc(&pdev->dev, sizeof(*serdes_gpio), GFP_KERNEL);
    if(!serdes_gpio)
        return -ENOMEM;
    ...
    dev_info(serdes_gpio->dev->parent->parent, "%s serdes_gpio_probe successful, base = %d, ngpio = %d\n",
             serdes->chip_data->name, serdes_gpio->gpio_chip.base, serdes_gpio->gpio_chip.ngpio);
...
static const struct of_device_id serdes_gpio_of_match[] = {
    ...
    { .compatible = "ti,ds90uh981-gpio", },
    { .compatible = "ti,ds90uh983-gpio", },
    { .compatible = "aim,aim951x-gpio", },
    {  }
};
