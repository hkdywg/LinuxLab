                                ...
                                struct drm_connector *connector)
{
    ...
    connector->display_info.width_mm = serdes_panel_split->width_mm; 
    connector->display_info.height_mm = serdes_panel_split->height_mm; 
    drm_display_info_set_bus_formats(&connector->display_info, &bus_format, 1);

    mode = drm_mode_duplicate(connector->dev, &serdes_panel_split->mode);
    mode->width_mm = serdes_panel_split->width_mm; 
    mode->height_mm = serdes_panel_split->height_mm; 
...
static int serdes_panel_split_parse_dt(struct serdes_panel_split *serdes_panel_split)
{
    ...
    dev_info(dev, "panel size %dx%d, rate = %d, cnt = %d, ssc = %d\n",
             serdes_panel_split->width_mm, serdes_panel_split->height_mm,
...
static int serdes_panel_split_probe(struct platform_device *pdev)
{
    ...
    if(np) {
        serdes_panel_split->backlight = of_find_backlight_by_node(np);
        of_node_put(np);
        if(!serdes_panel_split->backlight)
...
static const struct of_device_id serdes_panel_split_of_match[] = {
    { .compatible = "maxim,max96781-panel-split" },
    { .compatible = "ti,ds90uh981-panel-split" },
    { .compatible = "ti,ds90uh983-panel-split" },
    { .compatible = "aim,aim951x-panel-split" },
    {  }
};
