/*
* serdes_irq.c 
*	Interrupt controller support for different serdes chips
*
...
static irqreturn_t serdes_bridge_lock_irq_handler(int irq, void *arg)
{
    ...
    SERDES_DBG_MFD("%s %s %s ret =  %d\n", __func__, dev_name(serdes->dev), serdes->chip_data->name, ret);
...
int serdes_irq_init(struct serdes *serdes)
{
    ...
    if(serdes->lock_gpio) {
        char *label = devm_kasprintf(serdes->dev, GFP_KERNEL,
    ...
    SERDES_DBG_MFD("serdes %s serdes_irq_init successful\n", serdes->chip_data->name);
