...
static void serde_bridge_enable(struct drm_bridge *bridge)
{
    ...
    if(serdes_bridge->panel)
        drm_panel_enable(serdes_bridge->panel);
...
static int serdes_bridge_probe(struct platform_device *pdev)
{
    ...
    if(serdes_bridge->sel_mipi) {
        ...
        SERDES_DBG_MFD("%s: type %d\n", __func__, serdes_bridge->parent->chip_data->connector_type);
    } else {
        serdes_bridge->base_bridge.type = DRM_MODE_CONNECTOR_LVDS;
...
static const struct of_device_id serdes_bridge_of_match[] = {
    ...
    { .compatible = "ti,ds90uh981-bridge" },
    { .compatible = "ti,ds90uh983-bridge" },
    { .compatible = "aim,aim951x-bridge" },
    {  }
};
